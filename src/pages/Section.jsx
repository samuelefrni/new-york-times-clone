...
const Section = () => {
  ...
  const searchArticlesQuery = useQuery({
    ...
    queryFn: () =>
      searchArticlesRequest
        .get()
        .then((response) => setSectionArticle(response.data.results)),
  });
  ...
  if (searchArticlesQuery.isError)
    return (
      <>
        <section
          style={{
  ...
  return (
    <div className={ArticleCSS.articleContainer}>
      <Helmet>
        <title>{`Section: ${query.toUpperCase()}`}</title>
        <meta
          name="description"
          content={`News about: ${query.toUpperCase()}`}
        />
      </Helmet>
      {searchArticlesQuery.isSuccess &&
        sectionArticle &&
        sectionArticle.map((items, index) => {
          return (
            <article
              key={items.url}
              className={`${ArticleCSS.article} ${
                index % 4 === 0 ? ArticleCSS.fullWidth : null
              }`}
            >
              {items.abstract && (
